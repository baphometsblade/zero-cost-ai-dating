...
Writes are counted too, since round 25 — `harness.countingDb` tallies both halves of the
bill now. It bills what Firestore would: a refused write is not a write, so the tally moves
