...
the Firestore emulator, **188 checks** including the attacks each rule exists to stop. If you
change the data model, run it. See [`rules-tests/README.md`](../rules-tests/README.md).
`npm run test:emulator` adds the store suite to the same emulator boot: **211 more checks** that
...
```bash
npm test                    # 14 suites, 258 checks, no install required
...
match and the suite goes quiet again, all 258 checks passing, and the weakened policy deploys
