...
`npm run test:emulator` adds the store suite to the same emulator boot: **125 more checks** that
...
```bash
npm test                    # 14 suites, 239 checks, no install required
...
match and the suite goes quiet again, all 239 checks passing, and the weakened policy deploys
