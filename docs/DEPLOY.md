...
`npm run test:emulator` adds the store suite to the same emulator boot: **141 more checks** that
...
```bash
npm test                    # 14 suites, 246 checks, no install required
...
match and the suite goes quiet again, all 246 checks passing, and the weakened policy deploys
