...
| `specs/15-live-list.store.js` | the conversation list, pushed rather than polled: a page holding both the nav badge and the list pays for the rows ONCE, because they share one stream — so one conversation changing costs one read for both subscribers together rather than one each — plus one face per person per page, and nothing at all while nothing happens. Also the two things a composed listener gets wrong silently: a profile read that lands after a newer list must not put the older one back, and a hydration still resolving when the page tears down must not be delivered. And the failure both adapters used to lie about in opposite directions — a stream that dies now says so, to every subscriber, and a retry afterwards is allowed to work |
