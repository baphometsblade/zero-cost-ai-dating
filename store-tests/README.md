...
| `specs/15-live-list.store.js` | the conversation list, pushed rather than polled: a page holding both the nav badge and the list pays for the rows ONCE, plus one face per person per page, and nothing at all while nothing happens. What the shared stream does NOT do is save a read — the client SDK already shares one server target between listeners on exactly the same query, so the second subscription was always free, and the "two instead of one" this file reported was `countingDb` tallying per delivered callback. The checks say deliveries where they mean deliveries; see that spec's header. Also the two things a composed listener gets wrong silently: a profile read that lands after a newer list must not put the older one back, and a hydration still resolving when the page tears down must not be delivered. And the failure both adapters used to lie about in opposite directions — a stream that dies now says so, to every subscriber, and a retry afterwards is allowed to work |
